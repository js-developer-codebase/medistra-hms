"use client";

import React, { useEffect, useState, useMemo, Suspense } from "react";
import { useRouter, useSearchParams } from "next/navigation";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Select } from "@/components/ui/select";
import { Textarea } from "@/components/ui/textarea";
import { Badge } from "@/components/ui/badge";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from "@/components/ui/dialog";
import { useToast } from "@/components/ui/toast";
import {
  FileEdit,
  Search,
  Plus,
  RefreshCw,
  Download,
  Eye,
  Trash2,
  Calendar,
  User,
  CheckCircle2,
  Loader2
} from "lucide-react";

export default function ClinicalNotesPage() {
  return (
    <Suspense
      fallback={
        <div className="flex h-[450px] items-center justify-center">
          <Loader2 className="h-8 w-8 animate-spin text-emerald-600" />
        </div>
      }
    >
      <ClinicalNotesContent />
    </Suspense>
  );
}

function ClinicalNotesContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const initialPatientId = searchParams.get("patientId") || "";
  const { toast } = useToast();

  const [notes, setNotes] = useState<any[]>([]);
  const [patients, setPatients] = useState<any[]>([]);
  const [doctors, setDoctors] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);

  // Filters
  const [searchQuery, setSearchQuery] = useState("");
  const [categoryFilter, setCategoryFilter] = useState("ALL");

  // Create Modal
  const [createOpen, setCreateOpen] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [formData, setFormData] = useState({
    patient: initialPatientId,
    doctor: "",
    title: "",
    category: "SOAP Progress Note",
    chiefComplaint: "", // Subjective
    objectiveFindings: "", // Objective
    assessment: "", // Assessment
    plan: "", // Plan
    status: "Final"
  });

  // View Modal
  const [viewNote, setViewNote] = useState<any>(null);

  const loadData = async () => {
    try {
      const [recRes, patRes, docRes] = await Promise.all([
        fetch("/api/clinical/records"),
        fetch("/api/patient"),
        fetch("/api/user")
      ]);

      const [recData, patData, docData] = await Promise.all([
        recRes.json(),
        patRes.json(),
        docRes.json()
      ]);

      if (recData.success) {
        const all = recData.data || [];
        setNotes(all.filter((r: any) => r.recordType === "Clinical Note" || r.recordType === "Progress Note"));
      }
      if (patData.success) {
        setPatients(patData.data || []);
        if (initialPatientId) setCreateOpen(true);
      }
      if (docData.success) setDoctors(docData.data || []);
    } catch (err) {
      toast("Failed to load clinical progress notes", "error");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  };

  useEffect(() => {
    loadData();
  }, []);

  const handleRefresh = () => {
    setRefreshing(true);
    loadData();
  };

  const handleCreateSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.patient || !formData.title) {
      toast("Please select a patient and provide a note title", "error");
      return;
    }

    setSubmitting(true);
    try {
      const payload = {
        ...formData,
        recordType: "Clinical Note",
        dateRecorded: new Date().toISOString()
      };

      const res = await fetch("/api/clinical/records", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
      });

      const data = await res.json();
      if (res.ok && data.success) {
        toast("Clinical SOAP note recorded successfully!", "success");
        setCreateOpen(false);
        setFormData({
          patient: "",
          doctor: "",
          title: "",
          category: "SOAP Progress Note",
          chiefComplaint: "",
          objectiveFindings: "",
          assessment: "",
          plan: "",
          status: "Final"
        });
        loadData();
      } else {
        toast(data.error || "Failed to save note", "error");
      }
    } catch (err) {
      toast("An error occurred while saving clinical note", "error");
    } finally {
      setSubmitting(false);
    }
  };

  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this progress note?")) return;
    try {
      const res = await fetch(`/api/clinical/records/${id}`, { method: "DELETE" });
      const data = await res.json();
      if (res.ok && data.success) {
        toast("Note deleted successfully", "success");
        loadData();
      } else {
        toast(data.error || "Failed to delete note", "error");
      }
    } catch (err) {
      toast("Error deleting note", "error");
    }
  };

  const filteredNotes = useMemo(() => {
    return notes.filter((n) => {
      const q = searchQuery.toLowerCase().trim();
      const patName = (n.patient?.name || "").toLowerCase();
      const uhid = (n.patient?.uhid || "").toLowerCase();
      const title = (n.title || "").toLowerCase();
      const assessment = (n.assessment || "").toLowerCase();
      const docName = (n.doctor?.name || "").toLowerCase();

      const matchesSearch =
        !q ||
        patName.includes(q) ||
        uhid.includes(q) ||
        title.includes(q) ||
        assessment.includes(q) ||
        docName.includes(q);

      let matchesCat = true;
      if (categoryFilter !== "ALL") {
        matchesCat = n.category === categoryFilter;
      }

      return matchesSearch && matchesCat;
    });
  }, [notes, searchQuery, categoryFilter]);

  const exportCSV = () => {
    if (filteredNotes.length === 0) {
      toast("No notes to export", "error");
      return;
    }

    const headers = [
      "Date",
      "Patient Name",
      "UHID",
      "Title",
      "Category",
      "Subjective (S)",
      "Objective (O)",
      "Assessment (A)",
      "Plan (P)",
      "Attending Doctor"
    ];

    const rows = filteredNotes.map((n) => [
      `"${new Date(n.dateRecorded || n.createdAt).toLocaleDateString()}"`,
      `"${n.patient?.name || ""}"`,
      `"${n.patient?.uhid || ""}"`,
      `"${(n.title || "").replace(/"/g, '""')}"`,
      `"${n.category || "SOAP Note"}"`,
      `"${(n.chiefComplaint || "").replace(/"/g, '""')}"`,
      `"${(n.objectiveFindings || "").replace(/"/g, '""')}"`,
      `"${(n.assessment || "").replace(/"/g, '""')}"`,
      `"${(n.plan || "").replace(/"/g, '""')}"`,
      `"Dr. ${n.doctor?.name || ""}"`
    ]);

    const csvContent =
      "data:text/csv;charset=utf-8," + [headers.join(","), ...rows.map((e) => e.join(","))].join("\n");
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `Clinical_Notes_${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast("Clinical notes exported successfully", "success");
  };

  if (loading) {
    return (
      <div className="flex h-[450px] items-center justify-center">
        <Loader2 className="h-8 w-8 animate-spin text-emerald-600" />
      </div>
    );
  }

  return (
    <div className="space-y-6 pb-12">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
            <FileEdit className="h-6 w-6 text-cyan-600 dark:text-cyan-400" />
            Clinical Progress & SOAP Notes
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Document daily ward rounds, nursing observations, and structured SOAP (Subjective, Objective, Assessment, Plan) notes.
          </p>
        </div>

        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={handleRefresh}
            disabled={refreshing}
            className="flex items-center gap-1.5"
          >
            <RefreshCw className={`h-4 w-4 ${refreshing ? "animate-spin" : ""}`} />
            Refresh
          </Button>

          <Button
            variant="outline"
            size="sm"
            onClick={exportCSV}
            className="flex items-center gap-1.5"
          >
            <Download className="h-4 w-4" />
            Export CSV
          </Button>

          <Button
            size="sm"
            className="bg-cyan-600 hover:bg-cyan-700 text-white flex items-center gap-1.5"
            onClick={() => setCreateOpen(true)}
          >
            <Plus className="h-4 w-4" />
            New SOAP Note
          </Button>
        </div>
      </div>

      {/* Filter Bar */}
      <Card className="border shadow-sm">
        <CardContent className="p-3">
          <div className="flex flex-col sm:flex-row items-center gap-3">
            <div className="relative flex-1 w-full">
              <Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" />
              <Input
                placeholder="Search note title, patient, UHID, assessment, doctor..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="pl-9 h-9 text-xs"
              />
            </div>

            <div className="w-full sm:w-60">
              <Select
                value={categoryFilter}
                onChange={(e) => setCategoryFilter(e.target.value)}
                className="h-9 text-xs"
              >
                <option value="ALL">All Note Categories</option>
                <option value="SOAP Progress Note">SOAP Progress Note</option>
                <option value="Consultant Ward Round">Consultant Ward Round</option>
                <option value="Nursing Observation">Nursing Observation</option>
                <option value="Procedure Note">Procedure Note</option>
              </Select>
            </div>
          </div>
        </CardContent>
      </Card>

      {/* Notes Table */}
      <Card className="border shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold">Progress Notes Ledger</CardTitle>
          <CardDescription>
            Showing {filteredNotes.length} of {notes.length} documented progress notes
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="rounded-md border overflow-hidden">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>Date & Time</TableHead>
                  <TableHead>Patient Particulars</TableHead>
                  <TableHead>Category</TableHead>
                  <TableHead>Note Title</TableHead>
                  <TableHead>Assessment Summary</TableHead>
                  <TableHead>Attending Doctor</TableHead>
                  <TableHead className="text-right">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredNotes.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center text-slate-500 py-12 text-xs">
                      No progress notes found. Click "New SOAP Note" to document patient progress.
                    </TableCell>
                  </TableRow>
                ) : (
                  filteredNotes.map((n) => (
                    <TableRow key={n._id} className="text-xs hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                      <TableCell className="font-mono text-slate-500 whitespace-nowrap">
                        {new Date(n.dateRecorded || n.createdAt).toLocaleString([], {
                          dateStyle: "short",
                          timeStyle: "short"
                        })}
                      </TableCell>

                      <TableCell>
                        <div className="font-semibold text-slate-900 dark:text-white">
                          {n.patient?.name || "Unknown"}
                        </div>
                        <div className="text-[10px] text-slate-400 font-mono">
                          {n.patient?.uhid} • {n.patient?.gender}, {n.patient?.age}y
                        </div>
                      </TableCell>

                      <TableCell>
                        <Badge variant="outline" className="text-[10px]">
                          {n.category || "SOAP Note"}
                        </Badge>
                      </TableCell>

                      <TableCell className="font-bold text-slate-900 dark:text-white">
                        {n.title}
                      </TableCell>

                      <TableCell className="max-w-xs truncate text-slate-600 dark:text-slate-400">
                        {n.assessment || n.plan || "Assessment documented"}
                      </TableCell>

                      <TableCell className="font-medium">
                        Dr. {n.doctor?.name || "Attending"}
                      </TableCell>

                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 px-2 text-xs flex items-center gap-1 text-cyan-600 hover:text-cyan-700"
                            onClick={() => setViewNote(n)}
                          >
                            <Eye className="h-3 w-3" />
                            View
                          </Button>

                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 px-2 text-xs text-rose-600 hover:text-rose-700"
                            onClick={() => handleDelete(n._id)}
                          >
                            <Trash2 className="h-3.5 w-3.5" />
                          </Button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
            </Table>
          </div>
        </CardContent>
      </Card>

      {/* Add SOAP Note Modal */}
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <form onSubmit={handleCreateSubmit}>
            <DialogHeader>
              <DialogTitle className="text-base font-bold flex items-center gap-2">
                <FileEdit className="h-5 w-5 text-cyan-600" />
                Document Clinical SOAP Progress Note
              </DialogTitle>
              <DialogDescription>
                Structured Subjective, Objective, Assessment, and Plan documentation.
              </DialogDescription>
            </DialogHeader>

            <div className="space-y-4 py-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label htmlFor="patient" className="text-xs font-semibold">
                    Patient *
                  </Label>
                  <Select
                    id="patient"
                    value={formData.patient}
                    onChange={(e) => setFormData({ ...formData, patient: e.target.value })}
                    required
                    className="h-9 text-xs"
                  >
                    <option value="">-- Choose Patient --</option>
                    {patients.map((p) => (
                      <option key={p._id} value={p._id}>
                        {p.name} ({p.uhid || p.contact})
                      </option>
                    ))}
                  </Select>
                </div>

                <div className="space-y-1.5">
                  <Label htmlFor="doctor" className="text-xs font-semibold">
                    Attending Physician
                  </Label>
                  <Select
                    id="doctor"
                    value={formData.doctor}
                    onChange={(e) => setFormData({ ...formData, doctor: e.target.value })}
                    className="h-9 text-xs"
                  >
                    <option value="">-- Choose Doctor --</option>
                    {doctors.map((d) => (
                      <option key={d._id} value={d._id}>
                        Dr. {d.name}
                      </option>
                    ))}
                  </Select>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label htmlFor="title" className="text-xs font-semibold">
                    Note Title / Round Heading *
                  </Label>
                  <Input
                    id="title"
                    placeholder="e.g. Day 2 Post-Operative Ward Round, Morning ICU Assessment"
                    value={formData.title}
                    onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                    required
                    className="h-9 text-xs"
                  />
                </div>

                <div className="space-y-1.5">
                  <Label htmlFor="category" className="text-xs font-semibold">
                    Note Category
                  </Label>
                  <Select
                    id="category"
                    value={formData.category}
                    onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                    className="h-9 text-xs"
                  >
                    <option value="SOAP Progress Note">SOAP Progress Note</option>
                    <option value="Consultant Ward Round">Consultant Ward Round</option>
                    <option value="Nursing Observation">Nursing Observation</option>
                    <option value="Procedure Note">Procedure Note</option>
                  </Select>
                </div>
              </div>

              {/* S - Subjective */}
              <div className="space-y-1.5">
                <Label htmlFor="subj" className="text-xs font-semibold text-cyan-700 dark:text-cyan-400">
                  [S] Subjective — Patient Symptoms & Complaints
                </Label>
                <Textarea
                  id="subj"
                  rows={2}
                  placeholder="Patient reports pain reduced to 3/10. Slept well, tolerated oral fluids..."
                  value={formData.chiefComplaint}
                  onChange={(e) => setFormData({ ...formData, chiefComplaint: e.target.value })}
                  className="text-xs"
                />
              </div>

              {/* O - Objective */}
              <div className="space-y-1.5">
                <Label htmlFor="obj" className="text-xs font-semibold text-cyan-700 dark:text-cyan-400">
                  [O] Objective — Physical Exam & Observations
                </Label>
                <Textarea
                  id="obj"
                  rows={2}
                  placeholder="Afebrile, BP 120/80, HR 74. Surgical dressing clean and dry, no active oozing..."
                  value={formData.objectiveFindings}
                  onChange={(e) => setFormData({ ...formData, objectiveFindings: e.target.value })}
                  className="text-xs"
                />
              </div>

              {/* A - Assessment */}
              <div className="space-y-1.5">
                <Label htmlFor="ass" className="text-xs font-semibold text-cyan-700 dark:text-cyan-400">
                  [A] Assessment — Clinical Status & Impression
                </Label>
                <Textarea
                  id="ass"
                  rows={2}
                  placeholder="Stable post-operative recovery, resolving localized inflammation..."
                  value={formData.assessment}
                  onChange={(e) => setFormData({ ...formData, assessment: e.target.value })}
                  className="text-xs"
                />
              </div>

              {/* P - Plan */}
              <div className="space-y-1.5">
                <Label htmlFor="pl" className="text-xs font-semibold text-cyan-700 dark:text-cyan-400">
                  [P] Plan — Therapeutic Management & Orders
                </Label>
                <Textarea
                  id="pl"
                  rows={2}
                  placeholder="Step down IV analgesics to oral paracetamol. Mobilize out of bed with assistance..."
                  value={formData.plan}
                  onChange={(e) => setFormData({ ...formData, plan: e.target.value })}
                  className="text-xs"
                />
              </div>
            </div>

            <DialogFooter className="flex gap-2">
              <Button type="button" variant="outline" onClick={() => setCreateOpen(false)} disabled={submitting}>
                Cancel
              </Button>
              <Button type="submit" className="bg-cyan-600 hover:bg-cyan-700 text-white" disabled={submitting}>
                {submitting ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    Saving...
                  </>
                ) : (
                  <>
                    <CheckCircle2 className="mr-2 h-4 w-4" />
                    Save SOAP Note
                  </>
                )}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>

      {/* View SOAP Note Modal */}
      <Dialog open={!!viewNote} onOpenChange={() => setViewNote(null)}>
        <DialogContent className="max-w-xl">
          {viewNote && (
            <div className="space-y-4 text-xs">
              <DialogHeader>
                <DialogTitle className="text-base font-bold flex items-center gap-2">
                  <FileEdit className="h-5 w-5 text-cyan-600" />
                  {viewNote.title}
                </DialogTitle>
                <DialogDescription>
                  Recorded on {new Date(viewNote.dateRecorded || viewNote.createdAt).toLocaleString()} by Dr.{" "}
                  {viewNote.doctor?.name || "Attending"}
                </DialogDescription>
              </DialogHeader>

              <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg space-y-1">
                <div className="font-bold text-sm text-slate-900 dark:text-white">
                  {viewNote.patient?.name}
                </div>
                <div className="text-slate-500 text-[11px]">
                  UHID: {viewNote.patient?.uhid} • Category: {viewNote.category}
                </div>
              </div>

              <div className="space-y-3">
                <div className="p-2.5 rounded border border-slate-200 dark:border-slate-800">
                  <span className="font-bold text-cyan-700 dark:text-cyan-400 block text-[11px]">
                    [S] SUBJECTIVE
                  </span>
                  <p className="text-slate-800 dark:text-slate-200 mt-1 whitespace-pre-wrap">
                    {viewNote.chiefComplaint || "No subjective complaints noted"}
                  </p>
                </div>

                <div className="p-2.5 rounded border border-slate-200 dark:border-slate-800">
                  <span className="font-bold text-cyan-700 dark:text-cyan-400 block text-[11px]">
                    [O] OBJECTIVE
                  </span>
                  <p className="text-slate-800 dark:text-slate-200 mt-1 whitespace-pre-wrap">
                    {viewNote.objectiveFindings || "No objective findings noted"}
                  </p>
                </div>

                <div className="p-2.5 rounded border border-slate-200 dark:border-slate-800">
                  <span className="font-bold text-cyan-700 dark:text-cyan-400 block text-[11px]">
                    [A] ASSESSMENT
                  </span>
                  <p className="text-slate-800 dark:text-slate-200 mt-1 whitespace-pre-wrap">
                    {viewNote.assessment || "No assessment documented"}
                  </p>
                </div>

                <div className="p-2.5 rounded border border-slate-200 dark:border-slate-800">
                  <span className="font-bold text-cyan-700 dark:text-cyan-400 block text-[11px]">
                    [P] PLAN
                  </span>
                  <p className="text-slate-800 dark:text-slate-200 mt-1 whitespace-pre-wrap">
                    {viewNote.plan || "Continue current management"}
                  </p>
                </div>
              </div>

              <DialogFooter>
                <Button variant="outline" size="sm" onClick={() => setViewNote(null)}>
                  Close
                </Button>
              </DialogFooter>
            </div>
          )}
        </DialogContent>
      </Dialog>
    </div>
  );
}
